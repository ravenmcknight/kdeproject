...
\begin{equation}
  \hat{f}_n(x) = \frac{1}{nh} \sum_{i = 1}^{n} K\left(\frac{s - x_i}{h}\right)
\end{equation}
...
Consider 100 independent and identically distributed samples from an unknown distribution (plotted below). Given a bandwidth $h = 0.5$, write the Gaussian kernel density estimator for this sample at $s = 5$. Recall that the Gaussian PDF is $\frac{1}{\sqrt{2\pi \sigma^2}}exp(\frac{1}{2\sigma^2}(y-\mu))$.
...
ggplot(asimdat, aes(x=x)) +
  ...
  geom_density(bw = 0.5, kernel = "gaussian", color = "blue") + #closer to your estimate
 # stat_function(fun = my_est, color = "red") # your estimator! uncomment when you've defined my_est
  NULL
```
...
Then, we apply the 2nd order Taylor expansion for $f(hs + u)$ about $h = 0$. Omitting several steps of algebra, our Taylor expansion can be written
